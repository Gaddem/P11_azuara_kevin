...
const CompDropDownList = ({ title, content }) => {
  ...
  return (
    <div className="containerDropDownList ">
      <div
        className="rowContainDropDownList"
        onClick={() => setListeOpen(!listeOpen)}
      >
        <p className="titleDropDownList">{title}</p>
        <img
          className={`chevronDropDonwList ${
            listeOpen === true && "chevronDropDonwListUp"
          }`}
          src={chevron}
          alt="DropDown"
        />
